...
def reg (request):
    if request.method=="POST" and request.FILES['file']:
        ...
        if count>=1:
            ...
            content = "Thank you for registration in Kalarava website "
            mail = smtplib.SMTP('smtp.gmail.com', 587)
            mail.ehlo()
            mail.starttls()
            mail.login('kavyabhegde859@gmail.com', 'ignhyaqclqormgwx')
            mail.sendmail('kavyabhegde859@gmail.com', email, content)
            UserRegistration.objects.create(name=name,gender=gender,city=city,address=address,email=email,
                                            mobile_no=mobile_no,visitor_type=visitor_type,id_proof=id_proof)
...
def add_birds(request):
    cdata=BirdsCategory.objects.values('category_name').distinct()
    if request.method=="POST" and request.FILES['file']:
    ...
    return render(request,'add_birds.html',{'cdata':cdata})
...
def take_app(request):
    ...
    if request.method=="POST":
       ...
       content = "Thank you for Booking and your date is on-"+str(today)
       mail = smtplib.SMTP('smtp.gmail.com', 587)
       mail.ehlo()
       mail.starttls()
       mail.login('kavyabhegde859@gmail.com', 'ignhyaqclqormgwx')
...
def payment_report(request):
    udata=GenerateTicket.objects.all()
    return render(request,'payment_report.html',{'udata':udata})

def visitors(request):
    return render(request,'visitors.html')

def indian_visitors(request):
    count=UserRegistration.objects.filter(visitor_type='Indian').count()
    udata=UserRegistration.objects.filter(visitor_type='Indian').values()
    return render(request,'indian_visitors.html',{'count':count,'udata':udata})


def foreign_visitors(request):
    count = UserRegistration.objects.filter(visitor_type='Foreigner').count()
    udata = UserRegistration.objects.filter(visitor_type='Foreigner').values()
    return render(request, 'foreign_visitors.html', {'count': count, 'udata': udata})


def take_app_view_m(request):
    userdict = TakeAppointment.objects.filter(status='Accepted')
    return render(request,'take_app_view_m.html', {'userdict': userdict})
...
def pay(request,pk):
    ...
    request.session['adult_cost']=adult_cost
    request.session['children_cost']=children_cost
    ...
    if request.method=="POST":
        ...
        qcode = str(pk) + ".png"
        img.save("C:/Users/hp/projects/Birds_Sanctuary/Birdsanctuary/media/" + qcode)
    ...
    return render(request,'pay.html',{'amount':total,'adult_cost':adult_cost,'children_cost':children_cost})

def view_ticket(request,pk):
    count=GenerateTicket.objects.filter(appointment_id=pk).count()
    if count>=1:
        ...
        return render(request,'view_ticket.html',{'msg':'Still your request in Pending'})
...
def take_app_search(request):
    ...
    if request.method=="POST":
        ...
        #count=TakeAppointment.objects.filter(id=search).count()
        if id(search):
            ...
            return render(request, 'take_app_view_search.html', {'msg': 'Sorry..! Search Id'})
...
def take_app_del(request,pk):
    ...
    uid.delete()
    return redirect('take_app_status')
...
def edit_profile(request):
    username=request.session['username']
    userdict = UserRegistration.objects.filter(email=username).values()
    if request.method == "POST":
        name = request.POST.get('t1')
        gender = request.POST.get('r1')
        city = request.POST.get('t3')
        ...
        UserRegistration.objects.filter(email=username).update(name=name, gender=gender, city=city, address=address, email=email,
                                                      mobile_no=mobile_no, visitor_type=visitor_type)

        return redirect('reg_view')
    return render(request, 'edit_profile.html', {'userdict': userdict})
